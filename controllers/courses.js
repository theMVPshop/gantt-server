...
const getCoursesForCohort = (req, res) => {
  ...
  pool.query(sql, (err, rows) => {
    ...
    if (!rows.length)
      return res
        .status(404)
        .send(`No courses for cohort with id ${req.params.id}`);
    res.json(rows);
  });
  console.log("getCoursesForCohort");
};
...
const getCourse = (req, res) => {
  ...
  pool.query(sql, (err, rows) => {
    if (err) return handleSQLError(res, err);
    if (!rows.length)
      return res.status(404).send(`No course found with id ${req.params.id}`);
    res.json(rows);
  });
...
const createCourseForCohort = (req, res) => {
  ...
  console.log("createCourseForCohort");
};
...
const deleteCourse = (req, res) => {
  ...
  pool.query(sql, (err, rows) => {
    ...
    return res.send(
      `Deleted ${rows.affectedRows} course(s) with id ${req.params.id}`
    );
  });
